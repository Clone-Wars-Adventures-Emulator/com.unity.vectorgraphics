...
namespace Unity.VectorGraphics.Editor
{
    ...
    public class SVGImporter : ScriptedImporter, ISpriteEditorDataProvider
    {
        ...
        public override void OnImportAsset(AssetImportContext ctx)
        {
            ...
            switch (SvgType)
            {
                ...
                case SVGType.UISVGImage:
                    sprite = BuildSpriteFromGeometry(geometry, rect);
                    GenerateUGUISpriteAsset(ctx, sprite, name, sceneInfo);
                    break;
        ...
        private void GenerateUGUISpriteAsset(AssetImportContext ctx, Sprite sprite, string name, SVGParser.SceneInfo scene)
        {
            PrepareSpriteAsset(sprite, name);
            ...
            // TODO: wrap this in an if that checks if the SVG was generated by JPEXS, we only want to do this special code in our special use case
            // OR, we make this an optional import setting, TBD
            rt.sizeDelta = new Vector2(scene.SceneViewport.width, scene.SceneViewport.height);
            rt.pivot = createPivot(scene);
        ...
        private Vector2 createPivot(SVGParser.SceneInfo scene) {
            var root = scene.Scene.Root;
            if (root.Children == null || root.Children.Count == 0) {
                Debug.LogError($"Unsupported empty SVG {name}, pivot will be (0, 1)");
                return new Vector2(0, 1);
            }

            // in an SVG exported by JPEXS, this is the <g> tag that has the main transform applied for non images
            var gTag = root.Children[0];
            if (gTag.Children == null || gTag.Children.Count == 0) {
                Debug.LogError($"Unsupported empty group tag in SVG {name}, pivot will be (0, 1)");
                return new Vector2(0, 1);
            }

            // default the matrix to use the g tag's matrix
            Matrix2D m2d = gTag.Transform;

            // if the first child of the g tag has children, its likely a pattern tag, use it's matrix to determine the pviot instead
            if (gTag.Children[0].Children != null && gTag.Children[0].Children.Count > 0) {
                m2d = gTag.Children[0].Transform;
            }

            return new(m2d.m02 / scene.SceneViewport.width, 1 - (m2d.m12 / scene.SceneViewport.height));
        }
